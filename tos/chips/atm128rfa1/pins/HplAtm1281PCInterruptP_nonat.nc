...
implementation
{
  ...
  norace uint8_t irq_flag=0;
  norace uint8_t irq_flag_dir=0;
  norace bool signaling=FALSE;
  ...
  inline void signalfired(){
    while(irq_flag!=0){
      ...
      for(i=0;i<8;i++)
        if(irq_flag & (1<<i) )
          break;
      atomic{//irq between the two lines->signaling the penultimate direction
        toHigh=(irq_flag_dir&(1<<i))?TRUE:FALSE;
        irq_flag &= ~(1<<i);
      }
      switch(i){
        case 0:
