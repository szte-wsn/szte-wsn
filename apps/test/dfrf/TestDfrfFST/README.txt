...
This application tests the DFRF protocoll with Fat Spanning Tree Policy. A dedicated
...
back with it's nodeid (in this case, the rank is the grandparent of the node) with SpanningTreePolicy. 
Every node sends the receied dfrf messages to the serial port.

Usage:

- Program some node with the TestDfrfFST application, one with node ID 0, this
will be the sender.
...
The group ID is 0 if the message received with FatSpanningTreePolicy and 1 if it's
